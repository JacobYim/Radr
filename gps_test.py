...
from gps_serial_util import open_gps_serial, read_nmea_line
...
def parse_args():
    ...
    parser.add_argument(
        "--no-kernel-exclusive",
        action="store_true",
        help="Do not set TIOCEXCL (not recommended; allows multi-process port sharing).",
    )
...
def main():
    ...
    try:
        ser = open_gps_serial(
            ...
            args.baud,
            args.timeout,
            kernel_exclusive=not args.no_kernel_exclusive,
        )
